...
import com.xuecheng.base.constants.OrderTransactionStatus;
import com.xuecheng.base.constants.PaymentTransactionStatus;
...
public class OrderServiceImpl implements OrderService {
    ...
    @Autowired
    public OrderServiceImpl(XcOrdersMapper xcOrdersMapper,
                            XcPayRecordMapper xcPayRecordMapper,
                            XcOrdersGoodsMapper xcOrdersGoodsMapper,
                            MqMessageService mqMessageService,
                            RabbitTemplate rabbitTemplate) {
        this.xcOrdersMapper = xcOrdersMapper;
        this.xcPayRecordMapper = xcPayRecordMapper;
        this.xcOrdersGoodsMapper = xcOrdersGoodsMapper;
    ...
    public PayRecordDto createOrder(String userId, AddOrderDto addOrderDto) {
        ...
        if (payRecord == null) {
            XueChengPlusException.cast("添加支付交易记录失败");
            return null;
        }
        // 3. 生成二维码
        String qrCode = null;
    ...
    private XcPayRecord createPayRecord(XcOrders orders) {
        ...
        if (OrderTransactionStatus.PAID.getCode().equals(orders.getStatus())) {
        ...
        payRecord.setStatus(PaymentTransactionStatus.NOT_PAID.getCode());
    ...
    public PayRecordDto queryPayResult(String payNo) {
        ...
        OrderService proxy = (OrderService) AopContext.currentProxy();  // 代理对象解决事务失效问题
    ...
    private PayStatusDto queryPayResultFromAlipay(String payNo) {
        ...
        AlipayClient alipayClient = new DefaultAlipayClient(
                ...
                "json", AlipayConfig.CHARSET, ALIPAY_PUBLIC_KEY, AlipayConfig.SIGNTYPE);
    ...
    public void saveAlipayStatus(PayStatusDto payStatusDto) {
        ...
        // 2.1 已支付，直接返回
        if (OrderTransactionStatus.PAID.getCode().equals(statusFromDB)) {
            return;
        }
        // 3. 查询支付宝交易状态
    ...
    public void notifyPayResult(MqMessage mqMessage) {
        ...
        // 2. 设消息的持久化方式为 PERSISTENT，即消息会被持久化到磁盘上，确保即使在 RabbitMQ 服务器重启后也能够恢复消息。
        Message msgObj =
                MessageBuilder.withBody(jsonMsg.getBytes()).setDeliveryMode(MessageDeliveryMode.PERSISTENT).build();
        // 3. 封装 CorrelationData
        ...
        // 4. 发送消息
        rabbitTemplate.convertAndSend(MQConfig.PAYNOTIFY_EXCHANGE_FANOUT, "", msgObj, correlationData);
    }
